...
function Loader({ dimensions = 10, color = "#7289da" }: Props) {
  ...
  return (
    <Frame dimensions={dimensions} color={color} spin={spin}>
...
const Frame = styled.div<StyledProps>`
  ...
  @keyframes spinner-wandering-cubes {
    25% {
      -webkit-transform: translateX(${(props) => props.spin}) rotate(-90deg)
        scale(0.5);
      transform: translateX(${(props) => props.spin}) rotate(-90deg) scale(0.5);
    }
    50% {
      -webkit-transform: translateX(${(props) => props.spin})
        translateY(${(props) => props.spin}) rotate(-180deg);
      transform: translateX(${(props) => props.spin})
        translateY(${(props) => props.spin}) rotate(-180deg);
    }
    75% {
      -webkit-transform: translateX(0) translateY(${(props) => props.spin})
        rotate(-270deg) scale(0.5);
      transform: translateX(0) translateY(${(props) => props.spin})
        rotate(-270deg) scale(0.5);
    }
    to {
